...
from shared.messaging.events import INSTALLATION_ACTIVATION, WHISPER_INTERACTION, WHISPER_STATE, WHISPER_STATES, SemanticEvent
...
class TranslationSemanticIngress:
    ...
    def __init__(self, runtime, activation_topic: str, whisper_state_topic=None, whisper_interaction_topic=None, recent_ids=None):
        self._runtime, self._activation_topic, self._whisper_state_topic, self._whisper_interaction_topic = runtime, activation_topic, whisper_state_topic, whisper_interaction_topic
    ...
    def handle(self, topic: str, event: SemanticEvent, *, publish_authoritative=True, transport="unknown") -> bool:
        if topic == self._whisper_state_topic and event.event_type == WHISPER_STATE:
            ...
            return self._runtime.observe_whisper_state(state)
        if topic == self._whisper_interaction_topic and event.event_type == WHISPER_INTERACTION:
            if self._recent_ids.seen(event.id):
                return "ignored_duplicate"
            ...
            if source not in {"detector", "button"} or (source == "detector" and not isinstance(value, (int, float))) or (source == "button" and value is not None):
                return "rejected_invalid"
            return self._runtime.observe_whisper_interaction(event.payload)
    ...
    def handle_event(self, event: SemanticEvent, *, publish_authoritative=True, transport="unknown") -> bool:
        ...
        if event.event_type == WHISPER_STATE and self._whisper_state_topic:
            return self.handle(self._whisper_state_topic, event, transport=transport)
        if event.event_type == WHISPER_INTERACTION and self._whisper_interaction_topic:
            return self.handle(self._whisper_interaction_topic, event, transport=transport)
