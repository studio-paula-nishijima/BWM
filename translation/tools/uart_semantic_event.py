...
parser = argparse.ArgumentParser(); parser.add_argument("mode", choices=("send", "receive")); parser.add_argument("type", nargs="?", choices=("whisper.state", "installation.activation")); parser.add_argument("state", nargs="?"); parser.add_argument("--origin", default="uart_tool")
...
try:
    if args.mode == "send":
        event = whisper_state(args.origin, args.state) if args.type == "whisper.state" else installation_activation(args.origin, args.state)
